...
func (g *graph) entry() {
	...
	// XXX restore this behavior, probably by pulling it further up
...
func (g *graph) seeScope(node ast.Node, by types.Object) {
	...
	for _, name := range scope.Names() {
		...
		// Mark all local variables as used. We could track the usedness of local variables, and all existing code is
		// already set up to do so, but users don't expect to see unused local variables. We may want to flag local
		// variables eventually, but it'll require UI improvements.
...
func Graph(fset *token.FileSet,
	...
	g := newGraph(fset, files, pkg, info, directives, generated)
	g.entry()
	return g.nodes
}
